...
PostgreSQL.prototype.buildFields = function(model, data, excludeIds) {
  ...
  keys.forEach(function(field) {
    if (props[field]) {
      if (props[field].postgresql && props[field].postgresql.composite) {
  ...
  var fields = this._buildFieldsForKeys(model, data, nativeFields, excludeIds);
  for (var i = 0; i < compositeFields.length; i++) {
    ...
    if (data[field]) {
      ...
      fields.names.push(this.escapeName(field));
      fields.columnValues.push(new ParameterizedSQL(ParameterizedSQL.PLACEHOLDER, [null]));
    }
  }
  return fields;
}

PostgreSQL.prototype.buildReplaceFields = function(model, data, excludeIds) {
    var props = this.getModelDefinition(model).properties;
    var replacingData = {};
    Object.keys(props).forEach(function(field) {
        replacingData[field] = data[field] === undefined ? null : data[field];
    });
    return this.buildFields(model, replacingData, excludeIds);
}
...
PostgreSQL.prototype.columnEscaped = function(model, property) {
  if (isNested(property)) {
    ...
    var propertyDefinition = this.getPropertyDefinition(model, property)
    if (propertyDefinition.postgresql.composite) {
      propertyName = 'row_to_json(' + propertyName + ') AS ' + escapeIdentifier(property);
    }
    return propertyName;
  }
};
